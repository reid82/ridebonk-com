...
describe("POST /api/subscribe", () => {
  beforeEach(() => {
    ...
    // Keep process.env stubs so the process.env fallback path also works.
  ...
  it("returns 400 for malformed email", async () => {
    const res = await POST({
      request: makeRequest({ email: "nope" }),
      locals: runtimeLocals,
    } as never);
    expect(res.status).toBe(400);
  ...
  it("forwards a valid email through Resend and returns 200 (runtime.env path)", async () => {
    const res = await POST({
  ...
  it("forwards a valid email through Resend and returns 200 (process.env fallback)", async () => {
    // No locals.runtime.env — falls back to process.env stubs set in beforeEach.
    const res = await POST({
      request: makeRequest({ email: "rider@example.com", hp: "" }),
      locals: {},
    } as never);
